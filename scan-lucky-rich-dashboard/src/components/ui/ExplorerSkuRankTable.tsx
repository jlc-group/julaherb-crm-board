'use client'
// 📦 Explorer entity: สินค้า/SKU — ตารางจัดอันดับสไตล์ Kalodata
// แถวหลัก = per-day rows (ส่งมาจาก ExplorerTab · reliable) · daily-matrix เสริม sparkline/growth (ถ้ามี)
...
export default function ExplorerSkuRankTable({ perDayRows, loading, from, to, selectedSkus, sizeFilter }: {
  perDayRows: SkuRow[]; loading: boolean; from: string; to: string; selectedSkus: string[]; sizeFilter: SizeTier | 'all'
}) {
  const dm = useApi<SkuDailyMatrixResponse>(`/api/sku/daily-matrix?from=${from}&to=${to}`)

  // series รายวันต่อ SKU (สำหรับ sparkline + growth) — null ถ้า endpoint ยังว่าง
  const seriesBySku = useMemo(() => {
    ...
    for (const r of raw) {
      let e = m.get(r.sku); if (!e) { e = new Map(); m.set(r.sku, e) }
      e.set(r.date, (e.get(r.date) ?? 0) + r.scans)
    }
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    if (selectedSkus.length) base = base.filter((r) => selectedSkus.includes(r.sku))
  ...
  const columns: RankColumn<Row>[] = [
    ...
    { key: 'name', label: 'สินค้า', render: (r) => <span className="text-[var(--text-secondary)] block max-w-[180px] truncate" title={r.name}>{r.name}</span> },
  ...
  return (
    <div className="card p-4">
      <div className="flex items-center gap-2 mb-1 flex-wrap">
        ...
        <h3 className="text-[14px] font-bold text-[var(--dark)]">📦 ตารางจัดอันดับ SKU</h3>
