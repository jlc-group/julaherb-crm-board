...
เลือกแหล่งด้วย env `DATA_SOURCE` ใน [src/lib/api/adapter.ts](scan-lucky-rich-dashboard/src/lib/api/adapter.ts):

| ค่า | ความหมาย | สถานะ |
|-----|----------|-------|
| `mock` (ค่าเริ่มต้น) | อ่านจากไฟล์ static ใน `src/lib/` | ✅ ครบ 18/18 method |
| `api` | ยิงไป backend จริง **saversureV2** | ✅ 17/18 (ขาด time-of-day) |
| `db` | ต่อ Postgres ตรง | 🔴 **0/18 — ยังไม่ทำเลย** (มีแต่ comment SQL template ใน [db-source.ts](scan-lucky-rich-dashboard/src/lib/api/db-source.ts)) |
...
### A1. Channels — [ChannelsTab.tsx](scan-lucky-rich-dashboard/src/components/tabs/ChannelsTab.tsx)
...
### A2. Risk / Fraud — [RiskTab.tsx](scan-lucky-rich-dashboard/src/components/tabs/RiskTab.tsx)
...
### A3. Scan Behavior (รายละเอียด) — [ScanBehaviorTab.tsx](scan-lucky-rich-dashboard/src/components/tabs/ScanBehaviorTab.tsx)
...
|--------|-----------------|-------|--------------------------|
| **Time of Day** (Overview peak hours) | `/api/scans/time-of-day` | api-source คืน buckets ว่าง ([adapter stub](scan-lucky-rich-dashboard/src/lib/api/api-source.ts)) | endpoint แจกแจงตามชั่วโมง `[{hour, scans}]` |
...
|--------|--------|---------------------|
| Cohort retention (Customers) | [CustomersTab.tsx](scan-lucky-rich-dashboard/src/components/tabs/CustomersTab.tsx) | ตัวเลข hardcode ในไฟล์ |
| **Cross-scan pairs (SKU สแกนคู่กัน)** | [cross-scan-data.ts](scan-lucky-rich-dashboard/src/lib/cross-scan-data.ts) | **mock** ที่เพิ่งทำ — มี `TODO(api)` ในไฟล์ ชี้ให้ทำ `GET /api/sku/co-scan` (group `scan_log`/scan_history ตาม user แล้วนับคู่ SKU) |
| Cross-size matrix | `CROSS_SIZE_GROUPS` ใน [real-data.ts](scan-lucky-rich-dashboard/src/lib/real-data.ts) | static |
| Rank movement (Top10 ขยับ) | `RANK_MOVEMENT` ใน [daily-sku-data.ts](scan-lucky-rich-dashboard/src/lib/daily-sku-data.ts) | static 6 วัน (16–21 พ.ค.) |
| Engagement buckets / daily rollup fallback | `DAILY_ENTRIES` ใน [daily-update-data.ts](scan-lucky-rich-dashboard/src/lib/daily-update-data.ts) | static 16–24 พ.ค. (ใช้เป็น fallback ตอน mock) |
...
flow จับรางวัล → ประกาศ → ตรวจสิทธิ์ → ตรวจเอกสาร **ทั้งหมดเก็บเป็นไฟล์** ใน `data/` ของเครื่อง dashboard ผ่าน [claims-store.ts](scan-lucky-rich-dashboard/src/lib/claims-store.ts) (ใช้ `fs` ตรง ไม่ผ่าน adapter)
...
2. แก้ [claims-store.ts](scan-lucky-rich-dashboard/src/lib/claims-store.ts) ให้เรียก backend API แทน `fs.readFileSync/writeFileSync` (จุดเปลี่ยนรวมศูนย์อยู่ไฟล์เดียว)
...
**ไฟล์สำคัญสำหรับ dev:**
- [src/lib/api/adapter.ts](scan-lucky-rich-dashboard/src/lib/api/adapter.ts) — interface + ตัวสลับ source
- [src/lib/api/api-source.ts](scan-lucky-rich-dashboard/src/lib/api/api-source.ts) — client ต่อ saversureV2 (ตัวอย่างการ map)
- [src/lib/api/mock-source.ts](scan-lucky-rich-dashboard/src/lib/api/mock-source.ts) — รูปแบบ response ที่ทุก method ต้องคืน
- [src/lib/api/types.ts](scan-lucky-rich-dashboard/src/lib/api/types.ts) — contract ของแต่ละ response
- [src/lib/api/db-source.ts](scan-lucky-rich-dashboard/src/lib/api/db-source.ts) — โครง SQL template (ยังไม่ทำ)
- [src/lib/claims-store.ts](scan-lucky-rich-dashboard/src/lib/claims-store.ts) — จุดเดียวที่ต้องแก้เพื่อย้าย draw/claims ขึ้น DB
- [src/config/draw-rounds.ts](scan-lucky-rich-dashboard/src/config/draw-rounds.ts) — โครงสร้างรอบ/รางวัล/วันประกาศ
